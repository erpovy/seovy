...
const resetToDefaults = () => {
    if (!confirm('Tüm özellikleri ve başlıkları fabrika ayarlarına sıfırlamak istediğinize emin misiniz?')) {
        return;
    }
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-6">
            <div>
                <div class="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-xs font-semibold mb-2">
                    ...
                    <span>Özellikler Sayfası Yönetimi</span>
                </div>
                <h3 class="text-xl font-bold text-white tracking-tight">Landing Page & Özellikler Sayfası Özelleştirme</h3>
                <p class="text-xs sm:text-sm text-slate-400 mt-1 max-w-2xl">
                    Landing page ve <a href="/features" target="_blank" class="text-indigo-400 hover:underline inline-flex items-center space-x-0.5 font-medium">/features</a> sayfasındaki başlıkları, açıklamaları ve listelenen modül kartlarını yönetin.
                </p>
            </div>
        ...
        <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        <h4 class="text-sm font-bold text-white uppercase tracking-wider">Özellik Kartları ({{ form.features_list.length }})</h4>
                        <p class="text-xs text-slate-400 mt-0.5">Kartların sırasını değiştirebilir, aktif/pasif yapabilir veya yeni özellikler ekleyebilirsiniz.</p>
                    </div>
                    <button
                        type="button"
                        @click="addFeature"
                ...
                <div v-if="form.features_list.length === 0" class="text-center py-10 rounded-2xl bg-slate-950/40 border border-dashed border-slate-800 text-slate-500 text-xs">
                    Henüz hiçbir özellik tanımlanmamış. "Yeni Özellik Ekle" butonuna veya "Varsayılana Sıfırla"ya tıklayın.
                </div>
                    ...
                    >
                        ...
                        <div class="flex items-center justify-between border-b border-slate-800/60 pb-3">
                            <div class="flex items-center space-x-2">
                                ...
                                >
                                    {{ feature.is_active !== false ? 'Yayında' : 'Gizli' }}
                                </span>
                            </div>
                            ...
                            <div class="flex items-center space-x-1.5">
                                ...
                                <button
                                    ...
                                    :title="feature.is_active !== false ? 'Gizle' : 'Göster'"
                                >
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                            <div class="lg:col-span-2">
                                <label class="block text-[11px] font-semibold text-slate-400 mb-1">Özellik Başlığı</label>
                                <input
                            ...
                            <div>
                                <label class="block text-[11px] font-semibold text-slate-400 mb-1">Küçük Rozet (Opsiyonel)</label>
                                <input
                            ...
                            <div>
                                <label class="block text-[11px] font-semibold text-slate-400 mb-1">İkon Seçimi</label>
                                <select
                            ...
                            <div class="lg:col-span-3">
                                <label class="block text-[11px] font-semibold text-slate-400 mb-1">Açıklama</label>
                                <textarea
                            ...
                            <div>
                                <label class="block text-[11px] font-semibold text-slate-400 mb-1">Renk Teması</label>
                                <select
            ...
            <div class="pt-4 border-t border-slate-800 flex items-center justify-between">
                ...
                >
                    ...
                    <span>{{ (form.processing || isSaving) ? 'Kaydediliyor...' : 'Özellikleri ve Sayfayı Kaydet' }}</span>
                </button>
                ...
                >
                    <span>/features Sayfasını Önizle</span>
