    ...
    <div class="relative inline-block text-left" ref="dropdownRef">
        ...
        >
            <div class="flex items-center space-x-2 truncate">
                <span class="text-base leading-none">{{ activeLanguage.flag }}</span>
                <span v-if="!compact" class="truncate">{{ activeLanguage.nativeName }}</span>
        ...
        <div
            v-if="isOpen"
            class="absolute z-50 w-52 rounded-2xl bg-slate-900 border border-slate-800 shadow-2xl p-1.5 space-y-0.5 text-xs max-h-72 overflow-y-auto"
        ...
        >
            <div class="px-2.5 py-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                {{ activeLanguage.name === 'English' ? 'Select Language' : 'Dil Seçin / Language' }}
            </div>
